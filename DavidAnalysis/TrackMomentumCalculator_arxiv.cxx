#ifndef TRACKMOMENTUMCALCULATOR_ARXIV_CXX
#define TRACKMOMENTUMCALCULATOR_ARXIV_CXX

#include "TrackMomentumCalculator_arxiv.h"
// \file  TrackMomentumCalculator_arxiv.cxx
//
...
namespace larlite {
  ...
  TrackMomentumCalculator_arxiv::TrackMomentumCalculator_arxiv() {
    ...
    steps_size=5.0; n_steps=9; 
    
    for (Int_t i=1; i<=n_steps; i++) 
  ...
  Double_t TrackMomentumCalculator_arxiv::GetMuMultiScatterLLHD2(const larlite::mctrack &trk) {
    
    // By default, set the momentum to -1
    Double_t LLHD=-1.0;
    
    // Creating reco track vectors for each coordinate
    ...
    for (Int_t i=0; i<n_points; i++) {
      ...
      // Obtaining a vector of coordinates at a point of the track
      const TVector3 &pos=trk.at(i).Position().Vect();
      
      // Filling reco vector with positions from trk as it loops through (why?)
    ...
    // Obtaining size of the new reco vector; if value is basically 0, 1, or negative, it is "discarded"
    // Question: why is it only doing this for X?
  ...
  Int_t TrackMomentumCalculator_arxiv::GetRecoTracks(const std::vector<Float_t> &xxx, const std::vector<Float_t> &yyy, const std::vector<Float_t> &zzz) {
    ...
    // Assuming all track coordinate lengths match now, can loop through
    ...
    for (Int_t i=0; i<a1; i++) {
      ...
      // Filling reco vectors with these same values (why?)
      x_reco[n_reco]=nowx;
      y_reco[n_reco]=nowy;
      z_reco[n_reco]=nowz;
    ...
    // Why is the polyline ZXY rather than XYZ?
    gr_reco_xyz=new TPolyLine3D(n_reco, z_reco, x_reco, y_reco);
    
    // 2D graphs of different coordinates
  ...
  Int_t TrackMomentumCalculator_arxiv::GetSegTracks2(const std::vector<Float_t> &xxx, const std::vector<Float_t> &yyy, const std::vector<Float_t> &zzz) {
  ...
  Int_t TrackMomentumCalculator_arxiv::GetDeltaThetaij( std::vector<Float_t> &ei, std::vector<Float_t> &ej, std::vector<Float_t> &th, Double_t thick, std::vector<Float_t> &ind )
  {
  ...
  Double_t TrackMomentumCalculator_arxiv::my_mcs_llhd( Double_t x0, Double_t x1 )
  {
    Double_t p = x0;
	...
	Double_t TrackMomentumCalculator_arxiv::my_g( Double_t xx, Double_t Q, Double_t s )
	{
	  Double_t arg = 0.0;
  ...
  Double_t TrackMomentumCalculator_arxiv::find_angle( Double_t vz, Double_t vy )
  {
  ...
  Double_t TrackMomentumCalculator_arxiv::GetMomentumMultiScatterLLHD( const larlite::mctrack &trk )
  {
    Double_t p = -1.0;
  ...
  Double_t TrackMomentumCalculator_arxiv::GetMomentumMultiScatterLLHD( const larlite::track &trk )
  {
    Double_t p = -1.0;
  ...
  // Double_t TrackMomentumCalculator_arxiv::GetMomentumMultiScatterChi2( const larlite::mctrack &trk ){
  // 	  Double_t p = -1.0; 
