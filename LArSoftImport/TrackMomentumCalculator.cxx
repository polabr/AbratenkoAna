...
namespace larlite {
  ...
  Double_t TrackMomentumCalculator::GetMomentumMultiScatterLLHD(const larlite::mctrack &trk) {
    ...
    // Make recoX,Y,Z vectors of floats
    std::vector<Float_t> recoX; std::vector<Float_t> recoY; std::vector<Float_t> recoZ;
    
    recoX.clear(); recoY.clear(); recoZ.clear();
    
    // Get size of mctracks
    Int_t n_points = trk.size();
    ...
    for (Int_t i = 0; i < n_points; i++) {
      ...
      // After looping through all points in mctrack, recoX,Y,Z are full of all X,Y,Z coordinates in mctrack, respectively
    ...
    // Get value of segL at the final coordinate index
    ...
    // Why is end commented out and replaced with 0?
    Double_t start2 = 0.0; Int_t end2 = 0.0; // 800.0;
    
    for (Int_t k = start1; k <= end1; k++) {
    
      // This is eventually the momentum that is returned
      Double_t p_test = 0.001 + k * 0.01;
      
      // This part of the code isn't doing anything! (Loop from 0 to 0)
